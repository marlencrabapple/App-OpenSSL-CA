...
my $CATOP      = $ENV{CATOP}          // "/etc/ssl";
my $CAKEY      = $ENV{CAKEY}          // "cakey.pem";
my $CAREQ      = $ENV{CAREQ}          // "careq.pem";
my $CACERT     = $ENV{CACERT}         // "cacert.pem";
my $CACRL      = $ENV{CACRL}          // "crl.pem";
my @DAYS       = (split /,/, $ENV{DAYS})       // qw(-days 365);
my @CADAYS     = (split /,/, $ENV{CADAYS})     // qw(-days 1095);               # 3 years
my @EXTENSIONS = (split /,/, $ENV{EXTENSIONS}) // qw(-extensions v3_ca);
my @POLICY     = (split /,/, $ENV{POLICY})  // qw(-policy policy_anything);
my $NEWKEY     = $ENV{NEWKEY}         // "newkey.pem";
my $NEWREQ     = $ENV{NEWREQ}         // "newreq.pem";
my $NEWCERT    = $ENV{NEWCERT}        // "newcert.pem";
my $NEWP12     = $ENV{NEWP12}         // "newcert.p12";
...
sub split_val ($val, @pass) {
    return split_val_win32($val, @pass) if ( $^O eq 'MSWin32' );
    my ( @ret, @frag );
...
sub copy_pemfile ( $infile, $outfile, $bound, %opts ) {
    ...
    open my $IN, '<' . $opts{binmode}, $infile || die "Cannot open $infile, $!";
